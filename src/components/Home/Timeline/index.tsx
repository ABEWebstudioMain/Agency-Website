    ...
    <section ref={timelineRef} className="relative py-16 sm:py-20 lg:py-[100px]">
      <div className="px-4 xl:container">
        ...
        <div className="relative mx-auto max-w-6xl">
          {/* Timeline Line */}
          <div className="absolute left-1/2 top-0 h-full w-0.5 -translate-x-1/2 bg-stroke dark:bg-[#2E333D]">
            {/* Progress Indicator */}
            <div 
          ...
          {/* Scroll Tracker - "You are here" indicator */}
          <div 
            className="absolute left-1/2 z-10 flex -translate-x-1/2 items-center justify-center transition-all duration-300 ease-out"
            style={{ top: `${scrollProgress * 100}%` }}
          >
            <div className="flex items-center space-x-3 rounded-full bg-primary px-4 py-2 text-sm font-medium text-white shadow-lg">
            ...
            {timelineSteps.map((step, index) => (
              <div 
                ...
                className={`relative transition-all duration-500 ${
                  activeStep === index ? 'opacity-100 scale-100' : 'opacity-70 scale-95'
                }`}
              >
                {/* Time Label */}
                <div className="absolute left-1/2 top-0 z-20 -translate-x-1/2 -translate-y-1/2">
                  <div className="rounded-full bg-white px-4 py-2 font-heading text-sm font-medium text-dark shadow-md dark:bg-[#1D232D] dark:text-white">
                    {step.time}
                  </div>
                </div>
                ...
                <div className="grid gap-8 lg:grid-cols-2 lg:gap-16">
                  ...
                  <div className="lg:text-right">
                    <div className={`rounded-sm border-l-4 border-red-500 bg-red-50 p-6 transition-all duration-300 dark:bg-red-900/20 lg:p-8 ${
                      activeStep === index ? 'shadow-lg transform scale-105' : 'shadow-sm'
                    }`}>
                      <div className="mb-4 flex items-center lg:justify-end">
                        <div className="mr-3 flex h-10 w-10 items-center justify-center rounded-full bg-red-100 dark:bg-red-900/40 lg:order-2 lg:ml-3 lg:mr-0">
                          {step.painPoint.icon}
                        </div>
                  ...
                  {/* Solution (Right) */}
                  <div>
                    <div className={`rounded-sm border-l-4 border-green-500 bg-green-50 p-6 transition-all duration-300 dark:bg-green-900/20 lg:p-8 ${
                      activeStep === index ? 'shadow-lg transform scale-105' : 'shadow-sm'
                    }`}>
                      <div className="mb-4 flex items-center">
                        <div className="mr-3 flex h-10 w-10 items-center justify-center rounded-full bg-green-100 dark:bg-green-900/40">
                          {step.solution.icon}
                        </div>
