...
export default function AIOptimizationService() {
  ...
  const challenges = t('common.servicePages.aiOptimization.challenges.items').map((item: any, index: number) => ({
    icon: (
      <svg width="24" height="24" viewBox="0 0 24 24" className="fill-current text-orange-600">
        ...
        {index === 3 && <path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/>}
  ...
  const approachSteps = t('common.servicePages.aiOptimization.approach.steps').map((step: any, index: number) => ({
    ...
    icon: (
      <svg width="32" height="32" viewBox="0 0 32 32" className="fill-current text-primary">
        ...
        {index === 3 && <><path d="M16 2C8.268 2 2 8.268 2 16s6.268 14 14 14 14-6.268 14-14S23.732 2 16 2zm0 26C9.373 28 4 22.627 4 16S9.373 4 16 4s12 5.373 12 12-5.373 12-12 12z" /><path d="M12 14a2 2 0 100-4 2 2 0 000 4zM20 14a2 2 0 100-4 2 2 0 000 4zM16 24c4.418 0 8-2.686 8-6H8c0 3.314 3.582 6 8 6z" /></>}
  ...
  const benefits = t('common.servicePages.aiOptimization.benefits.items').map((benefit: any, index: number) => ({
    ...
    icon: (
      <svg width="32" height="32" viewBox="0 0 32 32" className="fill-current text-primary">
        ...
        {index === 1 && <><path d="M16 2C8.268 2 2 8.268 2 16s6.268 14 14 14 14-6.268 14-14S23.732 2 16 2zm0 26C9.373 28 4 22.627 4 16S9.373 4 16 4s12 5.373 12 12-5.373 12-12 12z" /><path d="M22.707 12.293l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L14 18.172l7.293-7.293a1 1 0 011.414 1.414z" /></>}
        {index === 2 && <><path d="M16 2C8.268 2 2 8.268 2 16s6.268 14 14 14 14-6.268 14-14S23.732 2 16 2zm0 26C9.373 28 4 22.627 4 16S9.373 4 16 4s12 5.373 12 12-5.373 12-12 12z" /><path d="M12 14a2 2 0 100-4 2 2 0 000 4zM20 14a2 2 0 100-4 2 2 0 000 4zM16 24c4.418 0 8-2.686 8-6H8c0 3.314 3.582 6 8 6z" /></>}
        {index === 3 && <><path d="M26 4H6a2 2 0 00-2 2v20a2 2 0 002 2h20a2 2 0 002-2V6a2 2 0 00-2-2zM6 26V6h20v20z" /><path d="M8 22h3v-8H8zM13 22h3v-12h-3zM18 22h3v-6h-3zM23 22h1v-10h-1z" /></>}
        {index === 4 && <path d="M16 2l4.12 8.36L30 12.24l-7 6.82 1.65 9.54L16 24.77l-8.65 3.83L9 18.06l-7-6.82 9.88-1.88L16 2z"/>}
        {index === 5 && <><path d="M16 2l-8 3v6c0 4.99 3.84 9.64 8 10.5 4.16-.86 8-5.51 8-10.5V5l-8-3z" /><path d="M14 18l-3-3 1.41-1.41L14 15.17l5.59-5.58L21 11l-7 7z" /></>}
      </svg>
    )
  }));
  ...
  const technologies = [
    {
      category: locale === 'de' ? "Machine Learning Bibliotheken" : "Machine Learning Libraries",
      items: ["TensorFlow", "PyTorch", "Scikit-learn"],
      color: "bg-blue-500"
    },
    {
      category: locale === 'de' ? "Natürliche Sprachverarbeitung" : "Natural Language Processing",
      items: ["SpaCy", "NLTK", "Hugging Face Transformers"],
      color: "bg-green-500"
    },
    {
      category: locale === 'de' ? "Cloud-KI-Services" : "Cloud AI Services",
      items: ["AWS SageMaker", "Google Vertex AI", "Azure AI"],
      color: "bg-purple-500"
    },
    {
      category: locale === 'de' ? "Datenverarbeitung" : "Data Processing",
      items: ["Pandas", "Apache Spark", "NumPy"],
      color: "bg-orange-500"
    },
    {
      category: locale === 'de' ? "Automatisierungs-Tools" : "Automation Tools",
      items: ["RPA Integration", "Custom Scripting", "Workflow Engines"],
      color: "bg-red-500"
    },
    {
      category: locale === 'de' ? "Integrationsmuster" : "Integration Patterns",
        ...
        <div className="px-4 xl:container">
          <div className="relative mx-auto max-w-4xl text-center">
            <div className="mb-6 inline-block rounded-full bg-primary/10 px-6 py-2 text-sm font-medium text-primary dark:bg-primary/20">
              {t('common.servicePages.aiOptimization.hero.badge')}
            </div>
            <h1 className="mb-8 font-heading text-4xl font-bold text-dark sm:text-5xl md:text-6xl dark:text-white">
              {t('common.servicePages.aiOptimization.hero.title').split(':')[0]}: <span className="bg-gradient-to-r from-primary to-primary/70 bg-clip-text text-transparent">{t('common.servicePages.aiOptimization.hero.title').split(':')[1]}</span>
            </h1>
            <div className="mx-auto max-w-3xl space-y-6 text-lg text-dark-text leading-relaxed">
              <p>
                {t('common.servicePages.aiOptimization.hero.description1')}
              </p>
              <p>
                {t('common.servicePages.aiOptimization.hero.description2')}
              </p>
            </div>
            ...
            <div className="mt-10 flex flex-wrap items-center justify-center gap-4">
              ...
              >
                {t('common.servicePages.aiOptimization.hero.cta')}
                <svg
                  width="16"
                  height="16"
                  viewBox="0 0 16 16"
              ...
              >
                {t('common.servicePages.aiOptimization.hero.seeIfYou')}
              </Link>
            </div>
          </div>
        </div>
          ...
          <div className="mx-auto max-w-6xl">
            <div className="mb-16 text-center">
              <h2 className="mb-6 font-heading text-3xl font-bold text-dark sm:text-4xl dark:text-white">
                {t('common.servicePages.aiOptimization.challenges.title')}
              </h2>
              <p className="text-lg text-dark-text">
                {t('common.servicePages.aiOptimization.challenges.subtitle')}
              </p>
            </div>
      ...
      <section className="bg-stroke/5 py-16 sm:py-20 lg:py-24 dark:bg-[#1D232D]">
        <div className="px-4 xl:container">
          <div className="mx-auto max-w-6xl">
            <div className="mb-16 text-center">
              <h2 className="mb-6 font-heading text-3xl font-bold text-dark sm:text-4xl dark:text-white">
                {t('common.servicePages.aiOptimization.approach.title')}
              </h2>
              <p className="text-lg text-dark-text">
                {t('common.servicePages.aiOptimization.approach.subtitle')}
              </p>
            </div>
      ...
      <section className="py-16 sm:py-20 lg:py-24">
        <div className="px-4 xl:container">
          <div className="mx-auto max-w-6xl">
            <div className="mb-16 text-center">
              <h2 className="mb-6 font-heading text-3xl font-bold text-dark sm:text-4xl dark:text-white">
                {t('common.servicePages.aiOptimization.benefits.title')}
              </h2>
              <p className="text-lg text-dark-text">
                {t('common.servicePages.aiOptimization.benefits.subtitle')}
              </p>
            </div>
        ...
        <div className="px-4 xl:container">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="mb-8 font-heading text-3xl font-bold text-dark sm:text-4xl dark:text-white">
              {t('common.servicePages.aiOptimization.whyChooseUs.title')}
            </h2>
            ...
            <div className="relative overflow-hidden rounded-sm bg-white/90 p-8 shadow-lg backdrop-blur-sm dark:bg-[#1D232D]/90 lg:p-12">
              <div className="relative space-y-6 text-lg text-dark-text leading-relaxed">
                <p>
                  {t('common.servicePages.aiOptimization.whyChooseUs.description1')}
                </p>
                <p>
                  {t('common.servicePages.aiOptimization.whyChooseUs.description2')}
                </p>
                <p>
                  {t('common.servicePages.aiOptimization.whyChooseUs.description3')}
                </p>
              </div>
      ...
      <section className="py-16 sm:py-20 lg:py-24">
        <div className="px-4 xl:container">
          <div className="mx-auto max-w-6xl">
            <div className="mb-16 text-center">
              <h2 className="mb-6 font-heading text-3xl font-bold text-dark sm:text-4xl dark:text-white">
                {t('common.servicePages.aiOptimization.technologies.title')}
              </h2>
              <p className="text-lg text-dark-text">
                {t('common.servicePages.aiOptimization.technologies.subtitle')}
              </p>
            </div>
        ...
        <div className="px-4 xl:container">
          <div className="relative mx-auto max-w-4xl text-center">
            <div className="mb-6 inline-block rounded-full bg-white/10 px-6 py-2 text-sm font-medium text-white">
              <span className="tracking-wide">{t('common.servicePages.aiOptimization.cta.badge')}</span>
            </div>
            <h2 className="mb-8 font-heading text-3xl font-bold leading-tight text-white sm:text-4xl sm:leading-tight lg:text-5xl lg:leading-tight">
              {t('common.servicePages.aiOptimization.cta.title')}
            </h2>
            <div className="mx-auto max-w-3xl space-y-6 text-lg text-white/90 leading-relaxed lg:text-xl lg:leading-relaxed">
              <p className="mb-10 text-lg leading-relaxed text-white/90 lg:text-xl lg:leading-relaxed">
                {t('common.servicePages.aiOptimization.cta.description')}
              </p>
            </div>
            ...
            <div className="flex flex-col items-center justify-center gap-4 sm:flex-row sm:gap-6">
              ...
              >
                {t('common.servicePages.aiOptimization.cta.consultation')}
                <svg
                  width="16"
                  height="16"
                  viewBox="0 0 16 16"
              ...
              >
                {t('common.servicePages.aiOptimization.cta.meetExperts')}
                <svg
                  width="16"
                  height="16"
                  viewBox="0 0 16 16"
            ...
            <div className="mt-8 text-sm leading-relaxed text-white/80">
              {t('common.servicePages.aiOptimization.cta.footer')}
            </div>
          </div>
        </div>
      </section>
